...
class FlightProcessor:
    ...
    def __init__(
        ...
        self._passed: list[dict[str, Any]] = []
    ...
    @property
    def passed_list(self) -> list[dict[str, Any]]:
        return self._passed
    ...
    def update_flights_in_area(self) -> None:
        ...
        if self._in_area is not None:
            ...
            self._passed = [v for k, v in current.items() if v.get('passed')] + [self._in_area[x] for x in exits if self._in_area[x].get('approaching')]
            self._event_manager.add_events(EVENT_ENTRY, self._entered)
            self._event_manager.add_events(EVENT_EXIT, self._exited)
            self._event_manager.add_events(EVENT_PASS, self._passed)
    ...
    def _update_flights_data(self,
        ...
        last_distance = previous_flight.get('distance') if previous_flight is not None else None
        last_approaching = previous_flight.get('approaching') if previous_flight is not None else True
